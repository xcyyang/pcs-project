...
func evaluatorMode(circ *circuit.Circuit, input *big.Int, once bool) error {
	ln, err := net.Listen("tcp", port) // create server
	if err != nil {
		return err
	}
...
func garblerMode(circ *circuit.Circuit, input *big.Int) error {
	nc, err := net.Dial("tcp", port) // connect server
	if err != nil {
		return err
	}
	conn := p2p.NewConn(nc)
...
func printResults(results []*big.Int, outputs circuit.IO) {
	for idx, result := range results {
		if outputs == nil {
			...
			// write output
			// res := printResult(result, outputs[idx], false)
			// data := []byte("{\"\"result\":\"}\"" + res)
			// err := os.WriteFile("../../public/json/mpc.json", data, 0644)
			// check(err)
