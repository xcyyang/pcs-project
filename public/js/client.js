...
var Client = (function(window) {
  ...
  var init = async function(config) {
      ...
      JunqiContract.events.FinishSetup({
          filter: {
              gameID: gameID
          },
          fromBlock: 0
              ...
              if (event['returnValues']['rankString'] == "-1") {
                  ...
                  if (!isMyself) {
                      ...
                      if (waitOpponentsRank) {
                          ...
                          JunqiContract.methods.move(event['returnValues']['moveString'], pieceSendToOpponent.rankStr, gameID).send({
                              from: currentAccount
                          });
                          // Update
                          game.move(event['returnValues']['moveString'], isMyself);
  ...
  var attachDOMEventHandlers = function() {
      ...
      container.on('click', '.valid-move', function(ev) {
          ...
          JunqiContract.methods.move(moveStringAndRankString.moveString, "-1", gameID).send({
              from: currentAccount
          });
          //messages.empty();
      ...
      container.on('click', '.valid-attack', function(ev) {
          ...
          // Send move message to Ethereum
          console.log(moveStringAndRankString);
          JunqiContract.methods.move(moveStringAndRankString.moveString, moveStringAndRankString.rankString, gameID).send({
              from: currentAccount
          });
      ...
      container.on('click', '#finishSetup', function(ev) {
          ...
          JunqiContract.methods.finishSetup(gameID).send({
              from: currentAccount
          });
      });
  ...
  var update = function() {
      ...
      if (gameState.status === 'nopieces') {
          if (opponent.hasMoveablePieces === false) {
              showGameOverMessage('nopieces-win');
          }
          if (you.hasMoveablePieces === false) {
              showGameOverMessage('nopieces-lose');
          }
      }

      // Test for forfeit
      if (gameState.status === 'forfeit') {
          if (opponent.forfeited) {
  ...
  var showGameOverMessage = function(type) {
      ...
      switch (type) {
          case 'checkmate-win':
              header.addClass('alert-success').text('Captured Flag');
              break;
          case 'checkmate-lose':
              header.addClass('alert-danger').text('Flag Lost');
              break;
          case 'forfeit-win':
              header.addClass('alert-success').text('Your opponent has surrendered');
              break;
          case 'forfeit-lose':
              header.addClass('alert-danger').text('You have surrendered');
              break;
          case 'nopieces-win':
              header.addClass('alert-success').text('Your opponent has no moveable pieces');
              break;
          case 'nopieces-lose':
              header.addClass('alert-danger').text('You have no moveable pieces left');
              break;
      }
